...
class qubit():
    def __init__(self,**kws):
        attribute = ['q_name','inst','T_bias','T_z','specfunc','specfunc_cavity','bias','zpulse','f_lo','delta','f_ex','delta_ex','alpha',\
...
async def executeZwave(measure,update_wave,dcstate={},calimatrix=None,**paras):
    ...
    for i in current:
        ...
        # task1.append(task_ready)
        task1[taskname] = task_ready
        task_manageawg = cww.couldRun(awg=awg,chlist=zch,namelist=zname)
        # task2.append(task_manageawg)
        task2[taskname] = task_manageawg
    return [task1,task2]
...
async def spec2d_awg(qubit,measure,freq,calimatrix,modulation=False):
    ...
    for i in current:
        task = await executeZwave(measure,cww.zWave,dcstate={qubit.q_name:i},\
            calimatrix=calimatrix,pi_len=27000/1e9,shift=100e-9)
        concurrence(task)
...
async def rabiPower(measure,amp,arg='amp',exstate=[]):
    ...
    for i in amp:
        ...
        pulselist = await cww.funcarg(cww.rabiWave,qubit,**para)
...
async def rabi_seq(measure,amp,arg='amp',exstate=[],comwave=True,readseq=True,mode='hbroadcast'):

    len_data, t = len(amp), np.array([amp]*measure.n).T
    task = await executeEXseq(measure,cww.Rabi_sequence,len_data,comwave,exstate,readseq,mode,v_or_t=amp,arg=arg)
    concurrence(task)
...
# async def rabi_many(measure,amp,arg='amp',exstate=[],comwave=True):

#     len_data, t = len(amp), np.array([amp]*measure.n).T
#     task = await executeEXseq(measure,cww.Rabi_sequence,len_data,comwave,exstate=exstate,v_or_t=amp,arg=arg)
#     concurrence(task)
...
async def pipulseOpt(measure,nwave,wavlen,optwhich='pi_len',exstate=[],mode='hbroadcast'):
    qubit = measure.qubits[exstate[0]]
    if optwhich == 'pi_len':
        pilen = qubit.pi_len
    ...
    for j,i in enumerate(range(nwave)):
        ...
        job = Job(mrw.rabi_seq, (measure,x,optwhich,exstate,True,readseq,mode),max=numrepeat,avg=avg,auto_save=False)
...
async def singleZpulse(measure,t_shift,exstate=[],comwave=True,updatez=True):
    ...
    if updatez:
        task = await executeZwave(measure,cww.zWave,dcstate={exstate[0]:0.005},pi_len=1000/1e9,shift=3000e-9)
        concurrence(task)

    job = Job(T1_seq, (measure,t_shift,exstate,comwave), tags=(exstate+['z_pulse']), max=300,avg=True)
    t_t, s_t = await job.done()
    yield t_t, s_t
...
async def zPulse(measure,t_T1,exstate=[]):
    ...
    zname, zch = [f'ch{i}' for i in qubit.inst['z_ch']], qubit.inst['z_ch']
    z_awg = measure.awg[qubit.inst['z_awg']]
    pulse1 = await cww.funcarg(cww.zWave,qubit,pi_len=1500/1e9,volt=0.05,shift=3000e-9)
    volt = np.linspace(-0.05,0.02,21)
    for i,j in enumerate(volt):
        pulse2 = await cww.funcarg(cww.zWave,qubit,pi_len=3500/1e9,volt=j,shift=1500e-9)
        pulse = np.array(pulse1) + np.array(pulse2)
...
async def vRabi(measure,t_rabi,v_rabi,dcstate=[],exstate=[]):
    len_data = len(t_rabi)
    ...
    for i, j in enumerate(v_rabi):
        ...
        task_z = await executeZseq(measure,cww.Z_sequence,len_data,True,dcstate=dcstate,v_or_t=t_rabi/1e9,arg='pi_len',volt=j,shift=300/1e9)
        concurrence(task_z)
        comwave = True if i == 0 else False
        job = Job(T1_seq, (measure,(t_rabi+600),exstate,comwave), tags=exstate, max=300,avg=True, no_bar=True)
        t_t, s_t = await job.done()
        yield [j]*measure.n, t_t, s_t    
# async def singleVrabi(measure,v_rabi,pi_len,dcstate):
#     len_data, t = len(v_rabi), np.array([v_rabi]*measure.n).T
#     task_z = await executeZseq(measure,cww.Z_sequence,len_data,True,dcstate=dcstate,v_or_t=v_rabi,arg='volt',pi_len=pi_len,shift=300/1e9)
#     concurrence(task_z)
#     await measure.psg['psg_lo'].setValue('Output','ON')
...
#         pulselist = await cww.funcarg(cww.rabiWave,qubit_ex,shift=(j+600)/1e9)
#         await cww.writeWave(awg_ex,name=namelist,pulse=pulselist)
#         await cww.couldRun(awg_ex,chlist,namelist)
#         # comwave = True if i == 0 else False
#         job = Job(singleVrabi, (measure,v_rabi,j/1e9,dcstate), tags=exstate, max=300,avg=True, no_bar=True)
#         t_t, s_t = await job.done()
#         yield [j]*measure.n, t_t, s_t    
